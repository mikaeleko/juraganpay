    ...
    <body>
                ...
                <div class="modal-body">
                    ...
                    <div style="padding-left:15%;padding-right:15%" align="center">
                        ...
                        <div style="text-align:right;">
                            ...
                            <div>
                                <input id="input-pin" type="password" class="form-input-password" style="font-familiy:Sogeo UI;font-size:18px;margin-right:0" placeholder="Masukan PIN">
                ...
                <div class="row">
                    <div class="col-desk-12">
                        <div class="navigation-feature">
                            ...
                            <button title="Paket Data" onclick="redirectTo('<?=base_url()?>belanja/paketdata')" class="button-image color-white"><img class="image-inner" src="<?php echo base_url() ?>images/asset/assetwebsite-02.png"></button>
                            <button title="Token Listrik" onclick="redirectTo('<?=base_url()?>belanja/tokenlistrik')" class="button-image color-white active-menu"><img class="image-inner" src="<?php echo base_url() ?>images/asset/assetwebsite-03.png"></button>
                            ...
                            <div class="row">
                                <div class="col-desk-12">
                                    <div>
                                        ...
                                        <p>Id Pelanggan <label style="color:green;font-size:12px"><br>*) masukan 10 atau 11 digit</label><img title="Nomor Favorit" src="<?=base_url() ?>images/asset/favorite-books.svg" onclick="openFavoriteBook()" class="button-favorite-numb"></p>

                                        <div style="position:relative">
                                            <span id="logo-operator"></span>
                                            <input id="input-no-tujuan" maxlength="12" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" onkeyUp="onkeyUp()" style="margin:0;width:100%; border:none; box-shadow: none;font-familiy:Sogeo UI;font-size:18px" type="number" class="form-input-number" placeholder="ketik disini"><hr>
                                        </div>
	    ...
	    var isExist = 0;
            function onkeyUp(){
                ...
                if((jumlahInput == 10 || jumlahInput == 11) && isExist == 0){
                    //get data
                    var t = getDateTime();
                    var h = sha1(hashtag + '<?=$this->session->userdata('lvl')?>' + '<?=$this->session->userdata('kd_price_plan')?>' + t);
                    ...
                    $.ajax({
                        ...
                        success: function(result){
                            ...
                            for(var i = 0;i < rs.length; i++){
                                ...
                                productItem += '<div onclick="selectionItem(this,'+rs[i].PRICE+','+i+',\''+rs[i].NOM+'\')" id="itemid'+i+'" class="product-name" style="width:170px;">'+
                                                    '<table>'+
                                                        '<tr>'+
                                                            '<td>'+
				...
				isExist = 1;
                                ...
                                logoOperator.innerHTML = '<img class="img-operator" alt="No img" src="<?=HOST_API_IMAGE?>get_img?file='+rs[0].IMG+'">'
                            }else{
			    	isExist = 0;
			    }
            ...
            function selectionItem(data,price,idxSelected,kdProduk){
                hargaProduk = price;
                kodeProduk = kdProduk;
            ...
            function selectionChildItem(data,price,kdProduk,denom){
                kodeProduk = kdProduk;
                ckKodeProduk.innerHTML = kdProduk;
            ...
            //     for(var i = 0;i < rs.length; i++){
            //         productItemChild += '<div onclick="selectionChildItem(this,'+rs[i].PRICE+',\''+rs[i].NOM+'\',\''+rs[i].DENOM+'\')" id="itemChildId'+i+'" class="product-name">'+rs[i].DENOM+'<br>('+rs[i].NOM+')<br><span class="font-product-price">Rp '+numberWithCommas(rs[i].PRICE)+'</span></div>'
            //     }
            //     divChildProduct.innerHTML = productItemChild;
            //     //jika jumlah child ada 1 otomatis seleksi
            //     if(rs.length == 1){
            //         var data = {id:'itemChildId0'};
            //         selectionChildItem(data,rs[0].PRICE,rs[0].NOM,rs[0].DENOM);
            //     }
            //     productItemChild = "";
            // }
