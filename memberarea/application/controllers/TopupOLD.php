...
class Topup extends CI_Controller {
  ...
  public function index(){
    // $get = $this->input->get();
    // $data['show'] = isset($get['show']) ? $get['show'] : '' ;

    $data['propinsi'] = $this->M_akun->get_propinsi();
    $data['akun']     = $this->M_akun->get_akun();
    $this->load->view('webreport_topup', $data);
  }
  ...
  function update_akun(){
    ...
    $MyHash = sha1($nama . $alamat_1 .$alamat_2 . $propinsi . $kota . $kecamatan . $kelurahan . $zip . $ktp . $npwp . $wp .$TIME);
