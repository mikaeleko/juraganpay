...
Class M_forgot extends CI_Model {
  ...
  function reset_code($data){
    ...
    // $sql = "UPDATE t_store_user SET WEB_PASS_RESET_CODE = e1(?), WEB_PASS_RESET_EXP=(sysdate+15/24/60)
  ...
  public function cek_reset_code($data) {
    ...
    //         and a.msisdn = ?
    //          ";
    //"SELECT user_name, full_name, d1(web_pass_reset_code) web_pass_reset_code, web_pass_reset_exp FROM t_store_user
    $sql = "SELECT user_name, full_name, web_pass_reset_code, web_pass_reset_exp FROM t_store_user
            WHERE store_id = ? AND user_name = ? ";

    // return $this->db->query($sql,[ $data['store_id'], $data['username'], $data['idforgot'] ])->row();
    $this->st24apiv1->set_host(HOST_API_INTERNAL);
    $this->st24apiv1->is_debug(false);
    $this->st24apiv1->set_query($sql,[ $data['store_id'], $data['username'] ]);
    $this->st24apiv1->set_secret(API_SECRET);
    $this->st24apiv1->run();
    return $this->st24apiv1->row();
  }
  ...
  function reset_pass($data){
    //$sql = "UPDATE t_store_user SET WEB_PASS = e1(?), WEB_PASS_RESET_CODE = '', WEB_PASS_EXPDATE=TO_DATE('01/01/2200 03:18:20','dd/mm/yyyy hh24:mi:ss'), WEB_PASS_RESET_EXP =''
