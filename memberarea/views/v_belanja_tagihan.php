    ...
    <body>
        <div class="container">
            <div id="activity-favorite" class="activity-no-favorite" style="position:fixed;width:300px;right:10px;top:80px">
                <div>
                    <div class="tool-bar" style="margin-top:20px;margin-bottom:5px;box-shadow: 0px 2px 2px 0px #e9e9e9">
        ...
        <script>
            ...
            var hashtag = "#<?=strtoupper($this->input->get('category'))?>";
            var dataProducts = [];
            ...
            function kirimTransaksiInquiry(price,type){
                ...
                if(type=="bpjs"){
                    noTujuan = inputNoVirAccKel.value;
                }else{
                    noTujuan = inputIdPelanggan.value;
                }
                ...
                if(noTujuan == ""){
                    ...
                    if(type=="bpjs"){
                        inqinputNoVirAccKel.focus();
                    }else{
                        inqinputIdPelanggan.focus();
                    }                    
                    return;
                }
                ...
                $("#err_msg").hide();
                var text = kodeProduk.replace('PAY','TAG.') + '.' + noTujuan + '.[[PIN]]';
                tempTextTag = text;
                ...
                for(var i = 0;i < dataSearchResultsLength; i++){
                    productItemSearch += '<div onclick="newActivityInquiry(\''+dataSearchResults[i].NOM+'\',\''+dataSearchResults[i].KET+'\',\''+dataSearchResults[i].FEE+'\',\''+dataSearchResults[i].DISCOUNT+'\',\''+dataSearchResults[i].IMG+'\''+dataSearchResults[i].PRICE+'\')" class="list-item">' +
                                                    ...
                                                    '<div class="left">'+
                                                        ...
                                                        '<span>'+(dataSearchResults[i].PRICE > 0?'Markup':'Fee')+ ' ' +dataSearchResults[i].HARGA.replace('-','')+'</span>'+
                                                    '</div>'+
                                                '</div>';
                }
