    ...
    if($pop_msg != ""){
     ...
     ?>
        ...
        <div id="data-cat" class="row" style="padding:30px;">
            ...
            <?php $count_parent_data = count($category->parent_data); ?>
           
            <div style="padding:10px;border-radius;5px;box-shadow: rgb(224, 224, 224) 1px 1px 6px 1px;margin-bottom:10px">
            <?= $title ?>
            <?php for($i=$count_parent_data-1;$i>=0;$i--){?>
                <?=$i==$count_parent_data-1?' > ':''?>
                <a href="<?=base_url().'category'.($category->parent_data[$i]->parent_category_id>0?'?parentid='.$category->parent_data[$i]->parent_category_id:'')?>"><?=$category->parent_data[$i]->product_category_name?></a> <?=($i > 0 ? ' > ':'')?>
            <?php } ?>
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      ...
                      <a title="lihat sub kategori <?= $cat->product_category_name ?>" href="<?=base_url()?>category?parentid=<?= $cat->product_category_id ?>"><?= $cat->product_category_name ?></a>
                    </td>
                    <td>
        ...
        <div id="add-cat" class="row" style="padding:30px;display:none;">
          ...
          <table>
              <tr>
                <input type="hidden" name="parent_category_id" value="<?=$this->input->get('parentid')?>">
                <td>Kategori</td>
